...
export default function Cart() {
  const { getUserCart, deleteItem, setnumsCartItems, clearCart, updateCartItem } =
    ...
    <>
      ...
      {cartData?.products.length > 0 ? (
                ...
                <div key={item._id} className="flex flex-col sm:flex-row items-center py-3">
                  <div className="w-full sm:w-10/12 flex flex-col sm:flex-row items-center">
                    ...
                    <div className="w-full sm:w-10/12 px-3 text-center sm:text-left">
                      ...
                      <h2 className="text-green-700 text-lg">Price: {item.price} EGP</h2>
                      <button
                        onClick={() => handleDeleteItem(item?.product?._id)}
                        className="border p-2 border-red-700 rounded text-red-700 hover:bg-red-700 hover:text-white mt-2 transition w-full sm:w-auto"
                      >
                        <i className="fa-solid fa-trash-can mr-2"></i> Remove
                      </button>
                    </div>
                  </div>
                  <div className="w-full sm:w-2/12 flex items-center justify-center mt-2 sm:mt-0">
                    <i
        ...
        <div className="flex flex-col justify-center items-center h-screen bg-gray-100 dark:bg-gray-800 px-4">
          ...
          <Link
            to="/"
            className="bg-green-700 hover:bg-green-800 text-white px-6 py-3 rounded-full text-xl transition w-full sm:w-auto text-center"
          >
            Start Shopping
          </Link>
        </div>
